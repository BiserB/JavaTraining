...
public final class CosmeticsEngine implements Engine {
    ...
    private String processSingleCommand(Command command) {
        switch (command.getCommandName()) {
            ...
            case "CreateCream":
                String creamName = command.getParameters().get(0);
                String creamBrand = command.getParameters().get(1);
                double creamPrice = Double.parseDouble(command.getParameters().get(2));
                GenderType creamGender = getGender(command.getParameters().get(3));
                Scent creamScent = Scent.valueOf(command.getParameters().get(4));
    ...
    private String createCream(String creamName, String creamBrand, double creamPrice, GenderType creamGender, Scent scent) {
        if (products.containsKey(creamName)) {
            return String.format(CREAM_ALREADY_EXIST, creamName);
        }

        Cream cream = factory.createCream(creamName, creamBrand, creamPrice, creamGender, scent);
        products.put(creamName, (Product) cream);
